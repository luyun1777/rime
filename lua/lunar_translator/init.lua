...
local function lunar_translator(input, seg, env)
	...
	if input == keyword then
		...
		local lunars = {
			{ t1 .. lunarJq.JQtest(os.date("%Y%m%d")), "〔公历⇉农历〕" },
			{ t1 .. lunarGz.GetLunarSichen(os.date("%H"), 1), "〔公历⇉农历〕" },
			{ t2 .. lunarJq.JQtest(os.date("%Y%m%d")), "〔公历⇉农历〕" },
...
-- 农历查询
local function queryLunar_translator(input, seg) --以任意大写字母(除U)开头引导反查农历日期，日期位数不足会以当前日期补全。
	local str, lunar
